...
def forecast(context, prediction_length, freq, metadata=None):
    ...
    if scale > 1e-6: # Only allow a trend if there's some variance in the recent data
        ...
        # Reduced from 0.1*scale (in template) to 0.075*scale for more conservatism.
    ...
    # phi = 0.6 is a conservative choice, ensuring rapid decay.
    phi = 0.6
    ...
    # This aligns with the recommendation to keep naive as the backbone and
    # apply only small, safe corrections. A high weight for naive ensures stability.
    weight_naive = 0.9  # Give 90% weight to the simple last-value forecast
    weight_damped_trend = 1.0 - weight_naive # 10% weight to the damped trend
