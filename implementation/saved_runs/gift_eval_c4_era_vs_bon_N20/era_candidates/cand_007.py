...
def forecast(context, prediction_length, freq, metadata=None):
    ...
    # Clip the slope to an even smaller fraction (5%) of the recent scale.
    # The parent candidate's MASE was worse than naive, suggesting its trend correction
    # was net negative. By reducing the maximum allowed trend contribution,
    # we make the forecast even more conservative and closer to the robust naive baseline.
    slope = float(np.clip(slope, -0.05 * scale, 0.05 * scale)) # Adjusted from 0.1 to 0.05
