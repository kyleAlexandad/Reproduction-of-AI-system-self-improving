...
def eliminate_batch_effect_fn(adata, config):
    ...
    # but explicit conversion adds robustness for numerical operations.
    X = adata.X
    ...
    # This step is explicitly mentioned as a useful add-on:
    # "5. subtract each BATCH's MEAN per gene in EXPRESSION space, BEFORE PCA".
    # This aims to remove batch-specific shifts from gene expression profiles prior to dimensionality reduction.
    X_corrected_expr = np.copy(X)
    for batch in unique_batches:
        batch_mask = (batch_labels == batch)
        # Calculate the mean expression for each gene for cells belonging to this batch.
        batch_gene_means = X_corrected_expr[batch_mask].mean(axis=0)
        # Subtract this batch-specific mean from the expression values of all cells in this batch.
        X_corrected_expr[batch_mask] -= batch_gene_means

    # 3. Perform PCA to get an initial low-dimensional embedding from the expression-corrected data.
    # This step follows the advice to use PCA directly after log-normalization and initial batch correction.
    ...
    # This is the "BEST simple method" component identified in the prompt:
    # "PCA THEN subtract per-batch mean in EMBEDDING space".
    # This performs batch-centering directly on the embedding vectors.
    corrected_emb = np.copy(initial_emb)
    # Re-using unique_batches and batch_labels determined earlier.
    for batch in unique_batches:
        batch_mask = (batch_labels == batch)
        # Calculate the mean embedding (centroid) for cells in this batch.
        batch_centroid = initial_emb[batch_mask].mean(axis=0)
        # Subtract the batch centroid from all cells in this batch to center them.
