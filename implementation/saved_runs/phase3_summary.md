...
3. **实现了 ERA 树搜索 vs best-of-N 独立采样 的对照实验**
   ...
   - **ERA 获胜**；并且 ERA 仅有 3/20 个失败候选，best-of-N 有 13/20 个失败候选，
     **ERA 明显更稳定**。
4. **新增重复运行评估与模型消融脚本**，用于把"单次结果"升级为更可信的小规模实验：
   - `repeat_era_vs_bon.py`：多次重复对照实验，输出 JSON / CSV / 曲线图 / 柱状图 / 中文小结。
   - `model_ablation.py`：`flash-lite` vs `flash` 的小型模型消融。
...
- **ERA vs best-of-N 是更有意义的下一步验证**：单次运行显示，在相同 LLM 调用预算下，
  ERA 不仅最终分数更高，而且失败候选远少于 best-of-N（因为 ERA 在"已经能跑通的父代码"
  上改进，而 best-of-N 每次都从弱基线从零生成复杂方案）。
  但单次运行噪声较大，**需要用重复实验（已实现）来确认这一优势是否稳定**。
...
| `saved_runs/repeated_era_vs_bon/` | 重复实验结果（运行 `repeat_era_vs_bon.py` 后生成） |
