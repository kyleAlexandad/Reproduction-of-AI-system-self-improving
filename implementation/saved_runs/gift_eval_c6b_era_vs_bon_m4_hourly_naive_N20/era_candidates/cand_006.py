...
def forecast(context, prediction_length, freq, metadata=None):
    ...
    if len(nan_indices_in_daily) > 0 and weekly_pattern_candidate is not None:
        ...
        # Also, check its length to ensure it's a full weekly cycle.
        if weekly_pattern_candidate is not None and len(weekly_pattern_candidate) == season_weekly and np.isfinite(weekly_pattern_candidate).any():
    ...
    # --- 7. Apply Damped Level Adjustment (Improvement) ---
    # This adjusts the overall level of the seasonal pattern based on the most recent context.
    # It adds a "damped level correction" as recommended in the prompt.
    adjusted_final_seasonal_pattern = np.copy(final_seasonal_pattern) # Start with the baseline pattern
    
    # We already have n >= season_daily check passed to reach this point.
    ...
    # This value is guaranteed to be finite because final_seasonal_pattern was nan_to_num'd.
    ...
    if np.isfinite(last_period_average):
        ...
        # This prevents over-reaction to short-term fluctuations or potential outliers in the last season.
        # A factor between 0 (no adjustment) and 1 (full adjustment).
        damping_factor = 0.7 
        damped_level_shift = level_difference * damping_factor
        
        # Add the damped level shift to the entire seasonal pattern.
        # Since adjusted_final_seasonal_pattern is finite and damped_level_shift is finite,
        # the result will also be finite.
