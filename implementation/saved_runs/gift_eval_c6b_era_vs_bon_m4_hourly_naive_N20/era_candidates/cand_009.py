...
def forecast(context, prediction_length, freq, metadata=None):
    ...
    # This provides a longer-term, potentially more robust daily pattern.
    ...
    # --- 7. Calculate Damped Level and Trend Components ---
    # These will be added to the base seasonal pattern to form the final forecast.
    ...
    if np.isfinite(last_season_context_avg):
    ...
    damped_trend_per_step = 0.0
    # Calculate trend if enough history for at least two full daily seasons.
    if n >= 2 * season_daily:
        prev_season_context_avg = np.nanmean(context[-2*season_daily:-season_daily])

        if np.isfinite(last_season_context_avg) and np.isfinite(prev_season_context_avg):
            # Calculate the raw trend (change in average level over one season).
            raw_trend_per_season = last_season_context_avg - prev_season_context_avg
            
            # Apply a damping factor to the trend to prevent overshooting.
            damping_factor_trend = 0.3 # A more conservative damping for trend.
            
            # Convert the damped trend from per-season to per-step.
            damped_trend_per_step = (raw_trend_per_season * damping_factor_trend) / season_daily
    ...
    # Add the damped level shift (constant for all steps) and the accumulated damped trend.
    for i in range(prediction_length):
        forecast_output[i] = tiled_seasonal_base[i] + damped_level_shift + (damped_trend_per_step * (i + 1))
